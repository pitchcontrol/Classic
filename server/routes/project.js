...
module.exports.save = function (req, res, next) {
    ...
    project.findOne({where: {name: req.body.projectName, user_id: user.id}}).then((prj) => {
        if (prj) {
            logger.warn('Такой проект уже есть: ' + req.body.projectName + ',' + prj);
            ...
            prj.save().then(function () {
                logger.info('Успешно сохранен проект: ' + prj.id);
                return res.json({id: prj.id});
            }).catch(function (error) {
                logger.error('Ошибка сохранения проекта ' + error);
...
module.exports.update = function (req, res, next) {
    ...
    project.findOne({where: {user_id: user.id, id: req.body.id}}).then((prj) => {
        if (prj) {
            logger.info('Обновляем проект, проект найден ид: ' + req.body.id + ', название: ' + prj.name);
            ...
            if (prj.name != req.body.projectName) {
                project.findOne({
                    ...
                    if (sp) {
                        ...
                        prj.save().then(function () {
                            logger.info('Успешно обновлен проект: ' + prj.id);
                            return res.json({id: prj.id});
                        }).catch(function (error) {
                            logger.error('Ошибка обновления проекта ' + error);
            ...
            logger.error('Попытка обновить проект: ' + req.body.id + ', проект не найден');
            return res.json({error: 'Проект не найден'});
        }
    }).catch(function (error) {
        logger.error('Ошибка обновления проекта, поиск проекта ' + error);
        return res.json({error: 'Ошибка обновления проекта'});
    });
};
module.exports.list = function (req, res, next) {
    ...
    project.findAll({where: {user_id: user.id}}).then((prj)=> {
        logger.info('Полученны проекты для : ' + user.id);
        return res.json(prj);
    });
};
module.exports.load = function (req, res, next) {
    ...
    project.findOne({where: {user_id: user.id, id: id}}).then((prj)=> {
        logger.info('Получен проект ид: ' + id);
        return res.json(prj);
    });
};
module.exports.delete = function (req, res, next) {
    ...
    project.findOne({where: {user_id: user.id, id: id}}).then((prj)=> {
        if (prj != null) {
            logger.info('Получен проект для удаления ид: ' + id);
            prj.destroy().then(()=> {
                logger.info('Проект удален: ' + id);
                return res.json({});
            });
        }
        else {
            logger.error('Попытка удалить проект: ' + id + ', проект не найден');
            return res.json({error: 'Проект не найден'});
        }
    });
};